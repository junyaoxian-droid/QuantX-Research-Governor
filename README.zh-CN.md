...
这是私有量化研究工作区所用「研究治理 skill」的可移植副本，附带配套的可复用协议、模板与脱敏范例。

本仓库是**下游**。`SKILL.md` 及其 references 的权威归属方是私有工作区`QuantX-Mac-Research`；这里是可以安装到别处、或在没有工作区权限时阅读的版本。修改先在上游做，再同步过来。

仓库不含策略逻辑、信号、持仓、数据集或收益承诺。不构成投资建议，也不是交易系统。

## 它解决什么问题

Agent 做研究会漂移：中途扩大范围、悄悄复用测试集、汇报最好的那个窗口而不是当初选中的那个、失败记录丢失。这个 skill 就是针对这些的刹车：
