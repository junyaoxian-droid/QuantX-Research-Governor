...
`candidate_freeze` and `recommendation` are explicit because Test access and
adoption boundaries cannot be validated without them.
...
3. Replace every synthetic value with evidence from the actual experiment.
4. Obtain real user authorization before `create_goal`; the fixture's simulated
   state is never authorization.
5. Append the canonical `00_RESEARCH_HUB/TEST_ACCESS_LEDGER.md` only when real
   Test computation occurs. The fixture entry is illustrative and is not a
   canonical access event.
6. Use the real workspace closeout and exact-path staging rules after work.
